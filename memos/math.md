...
- 수학적 귀납법(mathematical induction) : 자연수 $k$ 에 대한 명제 $p(k)$ 이 $n$ 번째 명제를 표시한다고 하자. 이때 모든 자연수 $n$ 에 대하여 아래 두 조건
  ...
  - 수학적 귀납법의 증명(귀류법)
    ...
    귀류법으로 증명하기 위하여 결론을 부정하면, 명제 $p(k)$ 를 거짓이 되게 하는 자연수 $k$ 가 존재한다. 그리고 집합 $A$ 를 

      $$ A = \{자연수 m | p(m) 은 거짓명제\} $$
    
    로 정의하자. 그러면 가정에 따라 $A$ 는 공집합이 아니다.
    
    $A$ 에 속한 원소 중 가장 작은 자연수를 $n$ 이라고 하자. 그렇다면 가정 1. 때문에 $n \geq 2$ 이다. 한편 $n-1$ 도 자연수인데 이것은 $n$ 보다 작기에 $A$ 에 속하지 않고, 이에 따라 명제 $p(n-1)$ 는 참이다. 
    
    그런데 가정 2. 로 인하여 명제 $p(n-1)$ 이 참이면 $p(n)$ 도 참이다. 이것은 $n \in A$ 와 모순이다. 따라서 처음의 가정은 거짓이며 이로써 수학적 귀납법이 증명되었다. 
