...
- 제곱 함수 $y = \{f(x)\} ^{n}$ 의 도함수 : $y' = n \{f(x)\} ^{n-1}f'(x)$

  - 합성함수 $y=g(f(x))$ 의 도함수 $y' = g'(f(x))f'(x)$ 에서 $g(x) = x ^{n}$ 로 두고 $x ^{n}$ 의 $x$ 를 $f(x)$ 로 둔 것이다. 
...
- 역함수의 도함수 : 직선의 방정식 $f(x)$ 위의 점 $(a, b)$ 과 점 $(c, d)$ 로 정의된 기울기 $f'(a) = \frac{d-b}{c-a}$ 이다.
  ...
  한편 직선을 $y=x$ 에 대하여 대칭시킨 역함수 의 점 $(b, a)$ 와 점 $(d, c)$ 역함수의 미분계수 $(f^{-1})'(b) = \frac{c-a}{d-b}$ 이다. 따라서 다음의 관계를 갖는다. 

  $$ f'(a) \circ (f ^{-1})'(b) = 1 $$

  이제 점 $(a, b)$ 를 그것이 속한 집합으로 회귀시키자. 즉, 이것을 일반화하여 임의의 점 $(x, f(x))$ 에서 
  ...
  $$ (f ^{-1} \circ f)(x) = x = f ^{-1}(t) $$

  이므로 

  $$ (f ^{-1})'(t) = \frac{1}{f'(f ^{-1}(t))} $$

  를 이다. 미지수 $t$ 를 보기 좋게 $x$ 로 바꾸면 최종적으로 
...
- 이계도함수(second order derivatives) : 함수 $f(x)$ 의 도함수 $f'(x)$ 가 미분가능할 때 $f'(x)$ 의 도함수를 함수 $f(x)$ 의 이계도함수라 한다. 
