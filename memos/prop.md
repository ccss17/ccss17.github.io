...
- 합의 법칙 : 사건 $A_1, A_2, \dots, A_n$ 이 동시에 일어나지 않을 때, 사건 $A_1, A_2, \dots, A_n$ 이 일어나는 경우의 수가 각각 $a_1, a_2, \dots, a_n$ 이면 사건 '$A_1, 또는 A_2, 또는 \dots A_n$ 이 일어나는 모든 경우의 수는 $\displaystyle  \sum_{k=1}^{n}a_k$ 이다. 
...
- 곱의 법칙 : 사건 $A_1, A_2, \dots, A_n$ 이 연이어 일어날 때, 사건 $A_1, A_2, \dots, A_n$ 이 일어나는 경우의 수가 각각 $a_1, a_2, \dots, a_n$ 이면 사건 $A_1, A_2, \dots, A_n$ 이 연이어 일어나는 모든 경우의 수는 $\displaystyle \prod_{k=1}^{n}a_k$ 이다. 
...
- 원순열 : 서로 다른 $n$ 개를 원형으로 배열하는 것이다. 

  - 즉 원순열은 
    
    $$ \frac{{}_{n}P_{n}}{n} = \frac{n!}{n} = (n-1)! $$
...
- 조합(combination) : 서로 다른 $n$ 개에서 순서를 생각하지 않고 중복되지 않게 $r$ 개를 택하는 것을 $n$ 개에서 $r$ 개를 택하는 조합이라 한다.
  ...
  - $\displaystyle \binom{n}{r}$ 로도 나타낸다. 
