...
- 중복조합(repeated combination) : 서로 다른 $n$ 개에서 순서를 생각하지 않고 중복을 허락하여 $r$ 개를 택하는 것을 서로 다른 $n$ 개에서 $r$ 개를 택하는 경우의 수
  ...
  - 증명 
    ...
    조합이란 서로 다른 $n$ 개에서 순서를 생각하지 않고 중복되지 않게 $r(0 \leq r \leq n)$ 개를 택하는 경우의 수였다.

    따라서 조합은 순열에 순서를 제거한 것이므로 

    $$ {}_{n}C_{r} = \frac{{}_{n}P_{r}}{r!} $$

    가 성립하고 
    
    순열은 조합에 순서를 부여한 것으로 볼 수 있으므로 선택한 $r$ 개를 일렬로 나열한 경우($r!$) 를 곱하여(곱의 법칙) 

    $$ {}_{n}P_{r} = {}_{n}C_{r} \times r! $$

    가 성립한다는 것을 우리는 이미 알고 있다. 
