...
VSC(Version Control System) 이란 파일의 변화를 추적하여 기록하는 프로그램이다. 이것을 사용하면 몇 가지 장점이 있는데 파일들을 버전으로 관리하여 복원할 때 편리하고 버전들을 쉽게 비교할 수 있다는 것이다. 

# git 

- 여러 VSC 프로그램이 있지만 git 이 그것들과 다른 점이 있다.

  - 여타 VSC 는 버전을 관리할 때 파일이 변경된 것을 중심으로 기록한다. 

  - 하지만 git 은 그 프로젝트 전체 파일의 스냅샷으로 버전을 관리한다. 물론 파일이 동일할 경우 이전 스냅샷에 단지 연결함으로써 용량을 아낀다! 

## git 이 파일을 관리하는 세 가지 상태

- git 은 modified, staged, committed 라는 3 가지 상태로 파일을 관리한다. 

  - modified : 파일을 변경했지만 아직 git 에 커밋되지 않은 상태이다. 

  - staged : 변경된 파일을 커밋이 될 리스트에 포함시킨 상태이다.

  - committed : git 데이터베이스에 하나의 버전으로 안전하게 저장된 상태이다. 
