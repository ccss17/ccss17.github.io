...
3. 이제 어떤 실수 $x=0.b_{1}b_{2}b_{3}b_{4}b_{5}\dots{}b_{i}\dots$ 를 이렇게 만들어보자. 실수 $x$ 의 소수 $i$ 번째 자리수 $b_i$ 를 위와 같은 실수 $R_i$ 에서 $a_{ii}$ 가 $9$ 보다 작은 수라면 $a_{ii}%2B1$ 로 정하고, $9$ 라면 $0$ 으로 정하자.
...
- 가령 **XS1T2>\*** 와 같이 표현된 매우 간단한 튜링기계를 생각하자. 그러면 이것을 다음과 같이 17진수 자연수로 해석하여 
    ...
    $$15\times{}17^{6}%2B10\times{}17^{5}%2B1\times{}17^{4}%2B11\times{}17^{3}%2B2\times{}17^{2}%2B12\times{}17^{1}%2B16\times{}17^{0}=376400467$$
