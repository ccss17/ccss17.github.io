...
- 하지만 이 정리로 인하여 초실수체가 실수체와 완전히 동일하게 작동한다고 볼 수 있는 것은 아니다. 
    ...
    가령 초실수체에서는 [아르키메데스 성질](../Foundations/numbers/#7ed1dd713)가 성립하지 않는다. 왜냐하면 양의 무한대는 $1$ 을 유한번 더한 값보다 항상 크기 때문이다. 
    
    또한 이 사실을 [아르키메데스 성질](../Foundations/numbers/#7ed1dd713)을 1차 논리로 기술할 수 없기 때문이라고도 설명할 수 있다. 1차 논리로 기술할 수 없기에 transfer principle 을 사용할 수 없다. 
