...
- 라그랑주 다항식의 정의에 의하여 자명하게 성립한다. [크로네터 델타의 정의](../LinearAlgebra/LinearTransformation/#059400286)를 참고하자.
...
- 증명
    ...
    스칼라 $c_0, c_1, \dots, c_n$ 에 대하여 $\displaystyle \bigg (\sum_{i=0}^{n} a_if_i\bigg )(c_j) = \sum_{i=0}^{n}a_if_i(c_j) = 0$ 이다. 한편 $f_i(c_j) = \delta _{ij}$ 이므로 $\displaystyle \sum_{i=0}^{n}a_if_i(c_j) = a_j$ 이다. 따라서 모든 $a_i$ 들은 $a_i = 0$ 이고 $\beta$ 는 일차독립이다. $\dim (\mathbf{P}_{n}(\mathbf{F})) = n + 1$ 이므로 [대체정리 따름정리 2](../LinearAlgebra/VectorSpace/#cd7879a47) 에 의하여 $\beta$ 는 $\mathbf{P}_{n}(\mathbf{F})$ 의 기저이다. ■ 
...
- 집합 $\beta = \{f_0, f_1, \dots, f_n\}$ 가 $\mathbf{P}_{n}(\mathbf{F})$ 의 기저이므로 $\mathbf{P}_{n}(\mathbf{F})$ 의 모든 다항식 $g$ 를 [정리 1.8](../LinearAlgebra/VectorSpace/#8a514fc5c) 에 의하여 다음과 같이 유일한 스칼라 $b_0, b_1, \dots, b_n$ 에 대한 $\beta$ 의 일차결합으로 표현할 수 있다.
