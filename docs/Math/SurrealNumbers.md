...
- 증명
    ...
    [정리 5](#c076cc283) 에 의하여 $\{|x_1\} < x_1$ 이고 $x_1$ 은 day $m$ 에서 가장 작은 초현실수이다. 그러므로 day $m+1$ 에서, 즉 [day $m$ 의 초현실수를 기반으로 생성할 수 있는 초현실 수](#464520693) 중 가장 작은 초현실수는 $\{|x_1\}$ 이다. 

    [정리 5](#c076cc283) 에 의하여 $x_n < \{x_n|\}$ 이고 $x_n$ 은 day $m$ 에서 가장 큰 초현실수이다. 그러므로 day $m+1$ 에서, 즉 [day $m$ 의 초현실수를 기반으로 생성할 수 있는 초현실 수](#464520693) 중 가장 큰 초현실수는 $\{x_n|\}$ 이다. ▲ 
...
$x_k$ 는 $x_k \leq x _{j-1}$ 을 만족하는 가장 오래된 초현실수이다.

$x_j$ 는 day $m$ 의 초현실수 $x_1 < x_2 < \dots < x _{n-1} < x_n$ 에서 $1<j \leq n$ 인 수이다. 

$x _{j-1}$ 는 $1 \leq j-1 \leq n-1$ 인 수이다. 