...
- 이 정리는 [정리 3.1.1 초른 보조정리](#b78f346ef)를 사용하여 증명되는데, 이런 증명 패턴이 수학의 여러 분야에서 전형적으로 쓰인다.
...
- 증명 
    ...
    그러면 $C$ 가 1) 을 만족하고 순서집합 $\mathcal{X}$ 가 [정리 3.1.1 초른 보조정리](#b78f346ef)의 가정을 만족하므로 $\mathcal{X}$ 에는 극대 원소 $B$ 가 존재한다. 이제 $B$ 가 2) 를 만족함을 보이면 증명이 끝난다.
