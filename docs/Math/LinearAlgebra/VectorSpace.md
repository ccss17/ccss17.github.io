...
- 이 정리는 벡터공간의 부분공간을 차원으로 분류할 수 있다는 것을 말해준다.

    가령 $\R ^{2}$ 은 $2$ 차원 벡터공간이므로 부분공간의 차원은 $0, 1, 2$ 이 될 수 있다. $0$차원 부분공간은 $\{0\}$ 이고, $2$차원 부분공간은 $\R ^{2}$ 이다. $1$차원 부분공간은 벡터 $v \in \R ^{2}$ 와 스칼라 $c \in \mathbf{F}$ 에 대한 집합 $\{c \cdot v : c \in \mathbf{F} \}$ 이다.
    ...
    마찬가지로 $\R ^{3}$ 의 부분공간의 차원은 $0,1,2,3$ 이 될 수 있고 $2$차원인 부분공간은 유클리드 공간에서 평면으로, $3$차원 부분공간을 유클리드 공간 전체로 볼 수 있다.
...
!!! def "라그랑주 다항식(Lagrange polynomial)"

    무한체 $\mathbf{F}$ 의 스칼라 $c_0, c_1, \dots, c_n$ 에 대하여 다음과 같이 정의한 다항식 $f_0, f_1, \dots, f_n$ 을 $c_0, c_1, \dots, c_n$ 에 대한 라그랑주 다항식이라 한다. 

    $$ f_i(x) = \dfrac{(x - c_0) \dots (x-c _{i-1})(x - c _{i+1}) \dots (x - c_n)}{(c_i - c_0) \dots (c_i-c _{i-1})(c_i - c _{i+1}) \dots (c_i - c_n)} = \prod_{\substack{k=0\\k \neq i}}^{n} \dfrac{x - c_k}{c_i - c_k} $$

!!! def ""

    무한체 $\mathbf{F}$ 의 스칼라 $c_0, c_1, \dots, c_n$ 에 대한 라그랑주 다항식 $f_0, f_1, \dots, f_n$ 에 대하여 다음이 성립한다.

    $$ f_i(c_j) = \delta _{ij} $$

- 라그랑주 다항식의 정의에 의하여 자명하게 성립한다. [크로네터 델타의 정의](../LinearTransformation/#059400286)를 참고하자.
...
- 증명
    ...
    스칼라 $c_0, c_1, \dots, c_n$ 에 대하여 $\displaystyle \bigg (\sum_{i=0}^{n} a_if_i\bigg )(c_j) = \sum_{i=0}^{n}a_if_i(c_j) = 0$ 이다. 한편 $f_i(c_j) = \delta _{ij}$ 이므로 $\displaystyle \sum_{i=0}^{n}a_if_i(c_j) = a_j$ 이다. 따라서 모든 $a_i$ 들은 $a_i = 0$ 이고 $\beta$ 는 일차독립이다. $\dim (\mathbf{P}_{n}(\mathbf{F})) = n + 1$ 이므로 [대체정리 따름정리 2](#cd7879a47) 에 의하여 $\beta$ 는 $\mathbf{P}_{n}(\mathbf{F})$ 의 기저이다. ■ 
...
- 집합 $\beta = \{f_0, f_1, \dots, f_n\}$ 가 $\mathbf{P}_{n}(\mathbf{F})$ 의 기저이므로 $\mathbf{P}_{n}(\mathbf{F})$ 의 모든 다항식 $g$ 를 [정리 1.8](#8a514fc5c) 에 의하여 다음과 같이 유일한 스칼라 $b_0, b_1, \dots, b_n$ 에 대한 $\beta$ 의 일차결합으로 표현할 수 있다.
...
- 예시 
    ...
    $$ \begin{equation}\begin{split} g(x) &= \sum_{i=0}^{2}g(c_i)f_i(x) = 8f_0(x) + 5f_1(x) -4f_2(x) \\ &= -3x ^{2}+6x+5 \end{split}\end{equation} \tag*{} $$

    세 점 $A, B, C$ 와 라그랑주 보간법으로 추정한 $g$ 를 기하학적으로 나타내보면 다음과 같다. 

    ![image](https://user-images.githubusercontent.com/16812446/131964624-1648bf27-fcba-42de-a186-455fea5e8a7f.png)
...
- (*보통 [하우스도르프 극대원리](../../Set/InfiniteSet/#a85bbef49)는 부분순서집합은 극대사슬을 가진다고 표현되지만, 이 책에서는 포함관계에 의한 부분순서관계를 표현하려고 집합족 $\mathcal{F}$ 의 임의의 사슬 $\mathcal{C}$ 에 대하여 $\mathcal{C}$ 을 포함하는 사슬 $U$ 가 존재한다고 말한 것 같다. 이렇게 말하면 집합족 $\mathcal{F}$ 에 포함관계에 의한 부분순서가 부여되고, 하우스도르프 극대원리에 의하여 $\mathcal{F}$ 에 극대원소가 있음을 말할 수 있다.*)
...
- 증명 (https://math.stackexchange.com/questions/52667/proof-that-two-basis-of-a-vector-space-have-the-same-cardinality-in-the-infinite)
    ...
    이다. 그런데 [임의의 무한기수 $a$ 에 대하여 $aa=a$ 이므로](../../Set/InfiniteSet/#530ae8378) $|I| \aleph _0 = |I|$ 이다. 그러므로 $|J| \leq |I|$ 을 얻는다. 같은 논리로 $|I| \leq |J|$ 를 얻는다. 그러므로 
