...
!!! tldr ""
    ...
    지표(characteristic) : 임의의 체 $\mathbf{F}$ 에서 $p = 0$ 인 가장 작은 자연수 $p$ 이다.
...
- 체 $\mathbf{F}$ 의 지표가 $p \neq 0$ 이면 
    ...
    $$ \forall x \in \mathbf{F}, x \cdot p = 0 $$
...
!!! tldr ""
    ...
    벡터공간(vector space) 또는 선형공간(linear space) : 체 $\mathbf{F}$ 에서의 벡터공간 $\mathbf{V}$ 는 다음 8가지 조건을 만족하는 두 연산 합 $+ : \mathbf{V} \times \mathbf{V} \to \mathbf{V}$ 과 스칼라곱 $\times : \mathbf{F} \times \mathbf{V} \to \mathbf{V}$ 을 가지는 집합이다. 

    1. $\forall  x, y \in \mathbf{V}, x+y=y+x$ (덧셈의 교환법칙)

    2. $\forall  x, y, z \in \mathbf{V}, (x+y)+z=x+(y+z)$ (덧셈의 결합법칙)

    3. $\forall x \in \mathbf{V}$ 에 대하여 $x+0=x$ 인 $0 \in \mathbf{V}$ 이 존재한다. (덧셈의 항등원)

    4. $\forall  x \in \mathbf{V}$ 에 대하여 $x+y=0$ 인 $y \in \mathbf{V}$ 가 존재한다. (덧셈의 역원)

    5. $\forall  x \in \mathbf{V}$ 에 대하여 $1x=x$ 이다. (곱셈의 항등원)

    6. $\forall a, b, x \in \mathbf{V}$ 에 대하여 $(ab)x = a(bx)$ 이다. (곱셈의 결합법칙)

    7. $\forall a, x, y \in \mathbf{V}$ 에 대하여 $a(x+y)=ax+ay$ 이다. (덧셈과 곱셈의 분배법칙)

    7. $\forall a, b, x \in \mathbf{V}$ 에 대하여 $(x+y)a=ax+ay$ 이다. (덧셈과 곱셈의 분배법칙)

- 합이란 $\mathbf{V}$ 의 두 원소 $x,y$ 에 대하여 유일한 원소 $x+y \in \mathbf{V}$ 를 대응하는 연산이다.

- 스칼라 곱이란 체 $\mathbf{F}$ 의 원소 $a$ 와 벡터공간 $\mathbf{V}$ 의 원소 $x$ 마다 유일한 원소 $ax \in \mathbf{V}$ 를 대응하는 연산이다.
...
!!! tldr ""
    ...
    $n$순서쌍($n$-tuple) : $a_1, a_2, \dots, a_n$ 이 체 $\mathbf{F}$ 의 원소일때 
    
    $$(a_1, a_2, \dots, a_n)$$

    라는 수학적 대상을 체 $\mathbf{F}$ 의 $n$순서쌍이라 한다.
...
!!! tldr "정리 1.1"
    ...
    벡터 합의 소거법칙 : 벡터 공간 $\mathbf{V}$ 에 대하여 
    
    $$x,y,z \in \mathbf{V}, x + z = y + z \implies x = y$$
...
- 임의의 벡터공간 $\mathbf{V}$ 에 대하여 $\{0\}$ 은 부분공간이다. 특히 $\{0\}$ 은 점공간(zero vector space)인 부분공간(zero subspace)이다.

- 벡터공간의 부분집합이 부분공간인지 확인하기 위해서 다음 성질만 확인하면 된다. 왜냐하면 벡터공간의 조건 1), 2), 5), 6), 7), 8) 은 자명하게 부분집합에서도 성립하기 때문이다. 

    1. $\forall x \in \mathbf{W}, y \in \mathbf{W}, x + y \in \mathbf{W}$

    2. $\forall c \in \mathbf{F}, x \in \mathbf{W}, cx \in \mathbf{W}$

    3. $0 \in \mathbf{W}$ ($\mathbf{W}$ 는 영벡터를 포함한다.)

    4. $\mathbf{W}$ 에 속한 모든 벡터의 덧셈에 대한 역벡터는 $\mathbf{W}$ 의 원소이다. 

- 다음 정리에 따르면 $\mathbf{W}$ 의 영벡터와 $\mathbf{V}$ 의 영벡터는 같다. 따라서 4) 는 확인할 필요 없다. 

!!! tldr "정리 1.3"

    부분공간의 조건 : 벡터공간 $\mathbf{V}$ 와 부분집합 $\mathbf{W}$ 에 대하여 $\mathbf{W}$ 가 다음 조건을 만족하면 부분공간이다.

    1. $0 \in \mathbf{W}$

    2. $\forall x \in \mathbf{W}, y \in \mathbf{W}, x + y \in \mathbf{W}$

    3. $\forall c \in \mathbf{F}, x \in \mathbf{W}, cx \in \mathbf{W}$

- 이때 조건 1), 2), 3) 의 연산은 $\mathbf{V}$ 에서 정의된 것과 같다.

- 증명 

    $\mathbf{W}$ 가 $\mathbf{V}$ 의 부분공간이면 $\mathbf{W}$ 는 $\mathbf{V}$ 의 합과 스칼라곱을 상속하므로 2), 3) 이 성립한다. $\mathbf{W}$ 의 영벡터를 $0'$ 라 하면 $x \in \mathbf{W}$ 에 대하여 $x + 0' = x$ 인데, $x \in \mathbf{V}$ 이므로 $x + 0 = x$ 이다. 따라서 $0 = 0'$ 이다. ▲ 

    역으로 1), 2), 3) 이 성립한다고 하자. 그러면 "d) $\mathbf{W}$ 에 속한 모든 벡터의 덧셈에 대한 역벡터는 $\mathbf{W}$ 의 원소이다." 만 확인하면 된다. 먼저 3) 에 의하여 $x \in \mathbf{W} \implies (-1) x \in \mathbf{W}$ 이고 $-x = (-1)x$ 이므로 $-w \in \mathbf{W}$ 이다. 그러므로 $\mathbf{W}$ 는 $\mathbf{V}$ 의 부분공간이다. ■ 

- 즉, 1), 2), 3) 으로부터 "d) $\mathbf{W}$ 에 속한 모든 벡터의 덧셈에 대한 역벡터는 $\mathbf{W}$ 의 원소이다." 가 도출되므로 이것을 확인할 필요 없다는 것이다.
...
!!! tldr ""
    ...
    일차종속(linearly dependent) : 벡터공간 $\mathbf{V}$ 의 부분집합 $S$ 에 대하여 $a_1u_1 + a_2u_2 + \dots + a_nu_n = 0$ 을 만족하는 임의의 서로 다른 벡터 $u_1, u_2, \dots, u_n \in S$ 와 적어도 하나는 $0$ 이 아닌 스칼라 $a_1, a_2, \dots, a_n$ 이 존재하면 집합 $S$ 를 일차종속이라 한다. 
...
!!! tldr "정리 1.7"
    ...
    3. 벡터공간 $\mathbf{V}$ 와 일차독립인 부분집합 $S$ 와 벡터 $v \in \mathbf{V} \text{ \textbackslash } S$ 에 대하여 $S \cup \{v\}$ 가 일차종속이기 위한 필요충분조건은 $v \in \text{span}(S)$ 이다. 
...
!!! tldr "정리 1.10"
    ...
    2. $\exists H \subset G \text{ s.t. } \text{span}(L \cup H) = \mathbf{V} \land \text{card}(H) = \text{card}(G) - \text{card}(L)$
...
- 이 정리에서 집합 $G$ 를 벡터공간 $\mathbf{V}$ 를 생성하는 극소 생성집합, 즉 기저라고 생각하는 것이 편하다. 왜냐하면 $\mathbf{V}$ 를 생성하는 임의의 생성집합 $G$ 에 대하여 
    ...
    $$ \text{card}(L) \leq \text{card}(G') = \dim(\mathbf{V} ) $$
    ...
    $$ \text{card}(L) \leq \text{card}(G') = \dim(\mathbf{V} ) \leq \text{card}(G) $$

    가 된다. 음.. 하지만 2) 의 경우 $G$ 에서 성립하므로

    $$\exists H \subset G \text{ s.t. } \text{span}(L \cup H) = \mathbf{V} \land \text{card}(H) = \text{card}(G) - \text{card}(L)$$
    ...
    $$ \exists S \subset \mathbf{V}, \text{card}(S) > \dim(\mathbf{V} ) \implies S \text{ is not linearly independent } $$
...
- [정리 1.10 "대체정리(replacement theorem) : 집합 $G$ 가 벡터공간 $\mathbf{V}$ 에 대하여 $\text{span}(G) = \mathbf{V}$ 이고 $L \subset \mathbf{V}$ 이 일차독립이면 다음이 성립한다. 1. $\text{card}(L) \leq \text{card}(G)$ 2. $\exists H \subset G \text{ s.t. } \text{span}(L \cup H) = \mathbf{V} \land \text{card}(H) = \text{card}(G) - \text{card}(L)$"](#e102d4179)
    ...
    "유한차원 벡터공간 $\mathbf{V}$ 에서 $\dim(\mathbf{V} )$ 보다 더 많은 개수의 벡터를 가지는 부분집합은 일차독립이 아니다."
...
- 즉, 유한차원 벡터공간 $\mathbf{V}$ 에서 $\dim(\mathbf{V} )$ 보다 더 많은 벡터를 가지면서 일차독립인 부분집합이 존재하지 않는다.
    ...
    $$ \not \exists S \subset \mathbf{V} \text{ s.t. } \text{card}(S) > \dim(\mathbf{V} ) \land S \text{ is linearly independent} $$
...
!!! tldr "정리 1.10 따름정리 2"
    ...
    1. $\mathbf{V}$ 의 임의의 유한 생성집합 $S$ 는 $\dim(\mathbf{V} ) \leq \text{card}(S)$ 이고, $\dim(\mathbf{V} ) = \text{card}(S)$ 인 생성집합 $S$ 는 $\mathbf{V}$ 의 기저이다.

    2. $S \subset \mathbf{V}$ 가 일차독립이고 $\text{card}(S) = \dim(\mathbf{V} )$ 이면 기저이다.
...
- 증명 
    ...
    이다. $\text{card}(L \cup H) \leq \text{card}(L) + \text{card}(H) = \text{card}(\beta) = \dim(\mathbf{V} )$ 이다. 1) 은 이미 증명되었으므로 1) 을 사용하면
    
    $$\dim(\mathbf{V} ) \leq \text{card}(L \cup H) \leq \dim(\mathbf{V} )$$

    이므로 $\text{card}(L \cup H) = \dim(\mathbf{V} )$ 이고, $L \subset L \cup H$ 는 $\mathbf{V}$ 의 기저가 된다. ■ 
...
!!! tldr "정리 1.11 따름정리"
    ...
    유한차원 $\mathbf{V}$ 의 벡터공간 $\mathbf{W}$ 에 대하여 $\mathbf{W}$ 의 임의의 기저를 확장하여 $\mathbf{V}$ 의 기저를 얻을 수 있다.
