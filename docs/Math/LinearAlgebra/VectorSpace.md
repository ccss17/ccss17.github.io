...
- 2), 3) 은 벡터공간의 두 연산이 닫혀있음을 나타내는 조건이다. 두 연산에 대하여 닫혀있다는 조건이 [벡터공간의 정의](#33d40353b) 에서는 합과 스칼라곱이 이항연산이라는 것에 함축되어 있다.

- 증명 

    벡터공간의 모든 벡터에 대하여 [벡터공간의 조건 1), 2), 5), 6), 7), 8)](#33d40353b) 이 성립하면 부분집합에서도 자명하게 성립한다. 그러므로 벡터공간의 부분집합이 부분공간임을 확인하기 위하여 [벡터공간의 조건 3), 4)](#33d40353b) 와 덧셈에 대하여 닫혀있다(조건 2)는 것과 스칼라 곱에 대하여 닫혀있다(조건 3)는 것만 확인하면 된다. 

    하지만 3) 에서 [벡터공간의 조건 4)](#33d40353b) 가 도출되므로 [벡터공간의 조건 4)](#33d40353b) 도 확인할 필요가 없다. 즉, $\mathbf{W}$ 에 속한 모든 벡터의 덧셈에 대한 역벡터는 $\mathbf{W}$ 의 원소라는 사실을 확인할 필요가 없다는 것이다. 이것을 먼저 증명해보자.
...
- 이 정리는 조건 1), 2), 3) 으로부터 [벡터공간의 조건 4)](#33d40353b) 이 도출되므로 [벡터공간의 조건 4)](#33d40353b) 를 굳이 확인할 필요가 없다는 것을 말해준다. 
...
- 증명
    ...
    $\beta$ 가 $\mathbf{V}$ 의 기저이고, $\gamma$ 가 $|\gamma| > |\beta|$ 인 $\mathbf{V}$ 의 기저라고 하면, $\exists S \subset \gamma \text{ s.t. } |S| = |\beta|+1$ 이다. $S$ 는 일차독립이고 $\text{span}(\beta) = \mathbf{V}$ 이므로 [대체정리](#dfa035489) 에 의하여 $|S| = |\beta| + 1 \leq |\beta|$ 인데, 이는 모순이다. 그러므로 $|\gamma| \leq |\beta|$ 이다. ▲ 
...
- 증명 
    ...
    $L \subset \mathbf{V}$ 이 일차독립이고 $|L| = \dim(\mathbf{V} )$ 라고 하자. [대체정리](#dfa035489) 에 의하여 
    ...
    $L \subset \mathbf{V}$ 가 일차독립이면 [대체정리](#dfa035489) 에 의하여 
...
- (*보통 [하우스도르프 극대원리](../../Foundations/InfiniteSet/#b2fd61dd6)는 부분순서집합은 극대사슬을 가진다고 표현되지만, 이 책에서는 포함관계에 의한 부분순서관계를 표현하려고 집합족 $\mathcal{F}$ 의 임의의 사슬 $\mathcal{C}$ 에 대하여 $\mathcal{C}$ 을 포함하는 사슬 $U$ 가 존재한다고 말한 것 같다. 이렇게 말하면 집합족 $\mathcal{F}$ 에 포함관계에 의한 부분순서가 부여되고, 하우스도르프 극대원리에 의하여 $\mathcal{F}$ 에 극대원소가 있음을 말할 수 있다.*)
...
- 이 정리는 [대체정리](#dfa035489) 을 무한차원으로 일반화한 것이다. 
