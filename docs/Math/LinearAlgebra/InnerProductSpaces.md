...
- 증명
    ...
    $n - 1$ 에서 정리가 성립함을 가정하고 $n$ 에 대하여 증명하자. 가정에 의하여 $\exists c, a_i \in \mathbf{F} : \det(A - tI_n) = c \displaystyle \prod_{i=1}^{n}(t-a_i)$ 이다. 

    $v_1$ 을 $A$ 의 고유벡터라고 하면 $Av_1 = \lambda _1 v_1$ 이 성립한다. 
