...
- $\mathbf{F}= \R$ 이면 켤레복소수를 생각할 필요 없다. 이 경우 우리가 이미 배웠을 가능성이 높은 $\big <x,y \big >$ 가 아니라 $x \cdot y$ 로 표기하는 내적과 같아진다.
...
!!! tldr "정규직교집합(orthonormal)"
    ...
    내적공간 $\mathbf{V}$ 의 부분집합 $S$ 가 단위벡터로 구성된 직교집합이면 정규직교집합이라 한다.
