...
!!! tldr "2.7"
    ...
    $$ n \in \N \implies \exists m \in \N \text{ s.t. } n = 0 \lor n = m ^{+} $$
...
!!! tldr "정리 2.2.4"
    ...
    1. $x>0 \implies  \exists n \in \N \text{ \textbackslash }\{0\} \text{ s.t.  }\  x > \dfrac{1}{n}$

    2. $y>0 \implies  \exists n \in \N \text{ \textbackslash }\{0\} \text{ s.t.  }\  y < n$
...
!!! tldr ""
    ...
    $\R$ 은 [정리 2.3.2 $A \neq \varnothing$ 인 집합 $A \subset \R$ 가 위로 유계이면 $A$ 는 최소상계를 갖는다](#febb2f17d) 를 만족시키는 순서체이다. 
...
- 증명 
    ...
    - 아 알았다.. 삼각 부등식 정리 "두 벡터 $x, y$ 에 대하여 $|x+y| \leq |x|+|y|$" 에 의하여 

        $$ |s_n| = |s_n - s_m + s_m| \leq |s_n - s_m| + |s_m| $$

        $$ \iff |s_n| - |s_m| \leq |s_n-s_m| < 1 $$
        ...
        $$ |x(i)| - |x(N)| \leq |x(i) - x(N)| < 1 $$

        가 되어 결국 

        $$ |x(i)| < |x(N)| + 1 $$

        이 되는거구나. 근데 $\leq$ 가 아니라 $<$ 로 쓰는게 맞는데 왜 $\leq$ 로 썼지.
...
!!! tldr "정리 2.4.5"
    ...
    $n \in \{1,2,\dots\}$ 에 대하여 $[ \alpha _n] \in \R$ 일 때 

    $$ n \mapsto [\alpha _n] $$

    을 $\R$ 의 코시수열이라고 하면, 이 수열은 $\R$ 안에서 수렴한다.

- 유리수의 코시수열이 유리수 내부에서 수렴하는 것이 아니라 무리수로 수렴하여서 새로운 수체계가 정의되었지만, 실수의 코시수열은 그렇지 않고 항상 실수 내부에서 수렴한다는 것을 말해주는 정리이다. 
...
!!! tldr ""
    ...
    완비순서체(completeness ordered field) 또는 실수의 완비성(completeness of the real numbers) : 순서체 $F$ 가 다음 세 동치 조건을 만족하면 이를 완비순서체라고 한다.
...
!!! tldr "정리 2.5.3"
    ...
    임의의 두 완비순서체 $F, G$ 가 주어지면 전단사 함수 $f: F \to G$ 가 존재하여 

    1. $\forall x,y \in F$ 에 대하여 $f(x+y)=f(x)+f(y), \quad f(xy)=f(x)f(y)$
...
- 이 정리는 완비순서체가 유일하게 존재함을 말한다. 즉, 완비순서체는 본질적으로 단 하나만 존재하고, 그것이 실수체 $\R$ 이라는 것이다. 만약 완비순서체의 조건을 만족하는 다른 체가 정의된다면, 그 체와 $\R$ 사이에 일대일 대응과 연산의 대응관계 같은 모든 구조를 그대로 보존하는 사상이 존재한다. 따라서 이 정리를 다음과 같이 이해하면 더욱 그 의미가 분명해질 것이다.
    ...
    임의의 완비순서체 $F$ 가 주어지면 전단사 함수 $f: F \to \R$ 가 존재하여 
