...
$a_2$ 가 $3$ 보다 크면 $n$ 에 $10$ 을 빼주어 $b_2$ 를 만든다. 

:   또 $a_2$ 으로 $n$ 이 $b_2$ 만큼 증가

$a_3$ 이 $3$ 보다 크면 $n$ 에 $100$ 을 빼주어 $b_3$ 를 만든다. 

:   또 $a_3$ 으로 $n$ 이 $b_3 \times (10 + 9)$ 만큼 증가

$a_4$ 이 $3$ 보다 크면 $n$ 에 $1000$ 을 빼주어 $b_4$ 를 만든다. 

:   또 $a_4$ 으로 $n$ 이 $b_4 \times (10 ^{2} - 10 \times 9 - 9 ^{2})$ 만큼 증가

즉, $a_k$ 가 $3$ 보다 크면 $n$ 에 $10 ^{k-1}$ 을 빼주어 $b_k$ 를 만든다. 

:   또 $a_k$ 로 인하여 $n$ 이 $b_k \times \bigg (\displaystyle \sum_{i=1}^{k-1}10 ^{k - 1 - i} \times 9 ^{i - 1}\bigg )$ 만큼 증가
...
- `f` 함수 : $O(\log_{10} n)$
    ...
    - (리커시브 함수의 시간 복잡도) = (함수 호출 횟수) * (각 호출마다 시간복잡도)
        ...
        입력 $n$ 에 대하여 함수가 $f(1), f(9), f(99), f(999), \dots, f(n)$ 와 같이 호출되므로(**메모이제이션 사용**) 
        
        함수 호출 횟수는 상수 $C_1$ 에 대하여 $k = C_1\log_{10} n$ 

        $k = C_1 \log_{10} n$ 번 반복되는 `for` 루프에서 `gamma` 가 상수 $C_2$ 에 대하여 
        
        입력 $i = C_2 \log_{10} n$ 으로 실행되어 시간 복잡도 $O(C_2 \log_{10} n)$ 로 실행 
        
        각 호출마다 시간복잡도는 $C_1 \log_{10} n \times C_2 \log_{10} n$

        $\therefore f(n)$ 의 시간 복잡도 : 
        
        $O(C_1 \log_{10} n \times C_1 \log_{10} n \times C_2 \log_{10} n) = O(3C_1 ^{2} C_2 \log_{10} n) = O(\log_{10} n)$ 
